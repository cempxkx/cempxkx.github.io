...
Class Master extends DBConnection {
	...
	// 		$code = sprintf("%'.04d",1);
	// 		while(true){
	// 			$check_code = $this->conn->query("SELECT * FROM payment where po_code ='".$prefix.'-'.$code."' ")->num_rows;
	// 			if($check_code > 0){
	// 				$code = sprintf("%'.04d",$code+1);
	// 			}else{
	// 				break;
	...
	// 	}
	// 	if(empty($id)){
	// 		$sql = "INSERT INTO `payment` set {$data}";
	// 	}else{
	// 		$sql = "UPDATE `payment` set {$data} where id = '{$id}'";
	// 	}
	// 	$save = $this->conn->query($sql);
	// 	if($save){
	// 		$resp['status'] = 'success';
	...
	// 	if($resp['status'] == 'success'){
	// 		if(empty($id)){
	// 			$this->settings->set_flashdata('success'," order was paid Successfully created.");
	// 		}else{
	// 			$this->settings->set_flashdata('success'," Purchase Order's Details Successfully updated.");
	// 		}
	// 	}
