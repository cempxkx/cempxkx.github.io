...
<!-- po-form keluar for created order, mention the generated po-code and supplier list -->
    ...
    <div class="card-body">
        <form action="" id="po-form">
            ...
            <div class="container-fluid">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <option value="<?php echo $row['id'] ?>" 
                                <?php echo isset($supplier_id) && $supplier_id == $row['id'] ? "selected" : "" ?> ><?php echo $row['name'] ?>
                            </option>
                            <?php endwhile; ?>
                            </select>
                        </div>
                ...
                <hr>
                <fieldset>
<!-- item for adalah untuk add new items or add items to the order ("New purchase order") -->
                    <legend class="text-info">Item Form</legend>
                    <div class="row justify-content-center align-items-end">
 <!--guna ni untuk db payment--><?php 
                                $item_arr = array();
                                $cost_arr = array();
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="py-1 px-4 text-center item">
                    ...
                    <tfoot>
                        ...
                        <!-- <tr>
                            <th class="text-right py-1 px-2" colspan="5">Shipping fee</th>
                            <th class="text-right py-1 px-2" colspan="5">8</th>
                        </tr> -->

                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">Discount Jadi Shipping 
                                <!-- <input style="width:40px !important" name="discount_perc" class='' type="number" min="0" max="100" value="<?php echo isset($discount_perc) ? $discount_perc : 0 ?>">% -->
                        ...
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">Total
                                <input type="hidden" id="amount"name="amount" value="<?php echo isset($discount) ? $discount : 0 ?>">
                            </th>
                ...
                <!-- try add form -->
                  <!-- <div class="card-body">
                    ...
                    <div class="col-md-10 mb-5">
                        <h2 class="text-center p-1 text-primary">Fill the details to complete your order</h2>
                        <form action="" id="payment-form" method="POST"> -->
                         <!-- <?php
                            // $qry = $conn->query("SELECT * FROM `payment` order by `name` desc");
                            // while($row = $qry->fetch_assoc()):
                            //     $row['payment_id'] = count(explode(',',$row['id']));
                                 
                            //      $payment_arr = array();
                            //     $payment_query = $conn->query("SELECT * FROM `payment` order by `name` asc");
                            //     while ($row = $payment_query->fetch_assoc()) {
                            //         $payment_arr[$row['payment_id']][] = $row;
                        ...
                        ?> -->
                         ...
                         <!-- <div class="form-group">
                            <input type="text" name="address1" class="form-control" placeholder="Enter your address 1" required>  
                            <!- try link dgn sales report--
                         </div>
                        <div class="form-group">
                            <input type="text" name="address2" class="form-control" placeholder="Enter your address 2" required>  
                            <!- try link dgn sales report --
                         </div>
                        <div class="form-group">
                            <input type="email" name="email" class="form-control" placeholder="Enter your email" required>  
                        <!- try add default form info -->
                        <!-- </div> 
                            
                                 <script>
                                //    src="https://checkout.stripe.com/checkout.js" class="stripe-button"
                                //    data-key = "sk_test_51NDrWoIA2wNZXULz4wzKgqiwxSxbuFIzqXCBG6Nsivh69mT4GqMpOBrQRdQGCJczS4kDEatHYv35xaRwgYaklVfe00cXLoQChm";
                                //    data-amount = <?php //echo str_replace(",","", $_GET["amount"])?>
                                //    data-name = "<?php //echo $_GET["item_id"]?>"
                                //    data-currency = "myr"
                                //    data-locale = "auto"
                                // </script> 
                         </div> 
    ...
    <div class="card-footer px-3 py-2 text-center">
        <button class="btn btn-flat btn-primary" type="submit" form="po-form">Create Order</button>
        <a class="btn btn-flat btn-primary"   href="<?php echo base_url.'admin/?page=payment/' ?>">Checkout-fill details</a>
        <!-- <a class="btn btn-flat btn-primary" type="submit" form="checkout-form" href="<?//php echo base_url.'admin/?page=payment/checkout' ?>">Checkout-Pay</a>  -->
    ...
    $(function(){
        ...
        $('#po-form').submit(function(e){
			...
			$.ajax({
                ...
                //try relate @ link kan dgn sales report
...
//------------------------------------------------------------------------------------------------------------
        ...
        //try coding (use discount as shipping fee)
        var sub_total = 0;
        var grand_total =0;
        var discount = 8;
        $('table#list tbody input[name="total[]"]').each(function(){
